...
or ASCII exodus file exported from the TRELIS to several mesh files required
by the SPECFEM3D package. Basic steps starting from the TRELIS:
in TRELIS:
- blocks define only the material regions
  -> actual material properties should not be defined within the TRELIS.
...
step3: produce mesh and BC files
  >>trelis2specfem3d tunnel.txt
  OR
  >>trelis2specfem3d tunnel.txt 1000.0
...
COMPILE:
  gcc trelis2specfem3d.c -o trelis2specfem3d
USAGE: 
  trelis2specfem3d <inputfile> <OPTIONS>
  Example: trelis2specfem3d tunnel.txt
  or
  trelis2specfem3d tunnel.e -fac=0.001 -bin=1
OPTIONS:
  ...
  -bin: use this option if you want to convert exodus binary directly, provided
        ...
        or -bin=0 for ascii file. [DEFAULT 0]
  -norm: use this option to check the normal of the faces. use -norm=1 for checking or -norm=0 for no checking [DEFAULT 0]
  -head: use this option no to attach head of input file to output file names. use -head=1 to attach header or -head=0 not to attach  [DEFAULT 0]
HISTORY:
...
fac=1.0; isbin=OFF; isnorm=OFF; ishead=OFF; ndir=1;
